  ...
  return (
    <div className="border rounded-lg p-6 bg-gray-50">
      ...
      <div className="flex gap-6">
        {/* 왼쪽: 이미지 업로드 */}
        <div className="w-1/3">
          <h5 className="text-sm font-medium text-gray-700 mb-3">이미지</h5>
          <ImageUploadField
            fieldName={fieldName}
            title={title}
            data={data}
        ...
        <div className="flex-1">
          <label className="block text-sm font-medium text-gray-700 mb-2">{title} 내용</label>
          <textarea
            value={data.content || ''}
            onChange={e => onContentChange(fieldName, e.target.value)}
          ...
          {fieldName === 'instagram' && data.content && data.content.trim() !== '' && (
            <div className="space-y-2 mt-3">
              <button
                type="button"
                onClick={() => onInstagramLink(data.content)}
              ...
              <button
                type="button"
                onClick={() => fetchInstagramThumbnail(data.content)}
