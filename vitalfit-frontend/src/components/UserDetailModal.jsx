        ...
        <div className="p-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-4">
              <h3 className="text-lg font-semibold text-gray-800 pb-2">기본 정보</h3>
            ...
            {/* 직무 정보 */}
            <div className="space-y-4">
              <h3 className="text-lg font-semibold text-gray-800 pb-2">직무 정보</h3>
                ...
                <div>
                  ...
                  <span
                    className={`px-3 py-1 rounded-full text-sm font-medium ${
                      user.position?.name?.toLowerCase().includes('관리자') ||
                      user.position?.name?.toLowerCase().includes('admin')
                        ? 'text-purple-600 bg-purple-50'
                        : user.position?.name?.toLowerCase().includes('매니저') ||
                            user.position?.name?.toLowerCase().includes('manager')
                          ? 'text-blue-600 bg-blue-50'
                          : user.position?.name?.toLowerCase().includes('트레이너') ||
                              user.position?.name?.toLowerCase().includes('trainer')
                            ? 'text-cyan-600 bg-cyan-50'
                            : user.position?.name?.toLowerCase().includes('직원') ||
                                user.position?.name?.toLowerCase().includes('staff')
          ...
          <div className="mt-6 space-y-4">
            <h3 className="text-lg font-semibold text-gray-800 pb-2">추가 정보</h3>
