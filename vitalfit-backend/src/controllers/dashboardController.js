...
const getDashboardStats = async (req, res) => {
  try {
    ...
    console.log('전체 멤버 (최근 10개):', allMembers.map(m => ({
      id: m.id,
      name: m.name,
    ...
    console.log('오늘 생성된 유저 데이터:', recentUsers.map(user => ({
      id: user.id,
      name: user.name,
      email: user.email,
    ...
    console.log('오늘 생성된 멤버 데이터:', recentMembers.map(member => ({
      id: member.id,
      name: member.name,
    ...
    console.log('오늘 생성된 공지 데이터:', recentNotices.map(notice => ({
      id: notice.id,
      title: notice.title,
